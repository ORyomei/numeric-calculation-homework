...
class NumericCalculation:
    ...
    def plot(self, axes: plt.Axes, color: str, label=""):
        # TODO 一般化
        data = np.genfromtxt(self.fileName, delimiter=",")
        axes.plot(data[:, 1], data[:, 2], marker=".",
        ...
        if self.fileName:
            with open(self.fileName, 'a') as f:
                ...
                while True:
                    ...
                    # v_ = self.v
                    self._update()
                    # if self.v[0] > 0.0 and v_[0] <= 0.0:
                    #     self.cycleTimes.append(self.t)
                    if count % logEvery == 0:
                        self.logData(writer)

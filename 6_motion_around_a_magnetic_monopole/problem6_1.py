...
axes1 = figure.add_subplot(121, title="Orbit of a charged particle around a magnetic monopole", projection="3d",
...
axes2 = figure.add_subplot(
    122, title="$v_0$-dependence of minimum distance", xlabel="$v_0$", ylabel="Minimum distance")
...
for v_0, color in zip(v_0s, colors):
    ...
    calculator.plot(axes1, color, collums=[
                    1, 2, 3], label="$v_0 = {}$".format(v_0))
    distanceMinimums.append(calculator.distanceMinimum)

axes2.scatter(v_0s, distanceMinimums, marker=".",
              color="b", label="Minimum distance")
v = np.linspace(0.0, 0.6, 100)
d = v / np.power(1 + v * v, 0.5)
axes2.plot(v, d, marker=".",
           markersize=1.0, linewidth=1.0, color="r", label=r"$\frac{v_{0}}{\sqrt{1+v_{0}^{2}}}$")
